...
import { ArrowUp, FileUp, Sparkles } from "lucide-react";
        ...
        {empty ? (
          <>
            ...
            <div className="flex min-h-0 flex-1 flex-col items-center justify-center px-8 pb-12">
              ...
              <div className="mt-8 w-full max-w-[720px]">
                <Composer input={input} busy={busy} onChange={setInput} onSend={send} large />
              </div>
              {starters.length ? (
          ...
          <>
            ...
            <div className="px-6 pb-6 pt-2">
              <Composer input={input} busy={busy} onChange={setInput} onSend={send} />
            </div>
          </>
        )}
      </section>
    ...
    >
      <div
        className={`border border-line bg-white ${
          large
            ? "rounded-[18px] p-4 shadow-[0_8px_30px_rgba(0,0,0,0.06)]"
            : "flex items-end gap-2 rounded-[24px] bg-[#f5f5f7] px-3 py-2"
        }`}
      >
        <textarea
          className={`w-full resize-none bg-transparent text-[15px] outline-none placeholder:text-faint ${
            large ? "min-h-[88px]" : "max-h-28 min-h-11 flex-1 px-2 py-2"
          }`}
        ...
        />
        <div className={`flex items-center justify-between ${large ? "mt-3" : ""}`}>
          {large ? (
          ...
          <button
            ...
            className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#e8e8ed] text-ink hover:bg-accent hover:text-white disabled:opacity-30"
          >
