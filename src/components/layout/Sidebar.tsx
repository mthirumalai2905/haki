...
export function Sidebar() {
  ...
  const hakiOpen = pathname === "/haki" || pathname.startsWith("/haki/");
    ...
    <aside className="relative z-30 flex w-[220px] shrink-0 flex-col bg-sidebar">
      ...
      <nav className="flex min-h-0 flex-1 flex-col gap-0.5 overflow-hidden px-2 pt-1">
        {items.map((item) => {
          if (item.href === "/haki") {
            return (
              <div key={item.href} className="min-h-0">
                <div
                  className={cn(
                    "relative z-10 flex items-center gap-2.5 rounded-[8px] px-2.5 py-1.5 text-[13px]",
                    hakiOpen ? "bg-white text-ink shadow-[0_1px_2px_rgba(0,0,0,0.06)]" : "text-muted hover:bg-white/60 hover:text-ink",
                  )}
                >
                  <Link
                    href="/haki"
                  ...
                  >
                    <MessageSquare className={cn("h-3.5 w-3.5", hakiOpen ? "text-accent" : "")} />
                    <span>Haki AI</span>
                  </Link>
                  <button
                    type="button"
                ...
                <div className="mt-0.5 max-h-[220px] space-y-0.5 overflow-auto pl-4">
                  {pathname === "/haki" ? (
