    ...
    <aside className="w-[320px] shrink-0 border-l border-line bg-surface">
      <div className="flex items-center justify-between border-b border-line px-4 py-3">
        <div>
          ...
          <div className="text-[11px] uppercase tracking-[0.12em] text-faint">{data.type}</div>
        </div>
        <button type="button" className="text-xs text-muted" onClick={onClose}>Close</button>
      </div>
              ...
              >
                {CHANNELS.map((channel) => (
                  <option key={channel.id} value={channel.id}>
                    {channel.label}{channel.implemented ? "" : " · coming soon"}
                  </option>
                ))}
              </select>
            </Field>
